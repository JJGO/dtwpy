...
def plot_cost_matrix(x,y,**kwargs):
    ...
    ax3.axis('off')
    ax3.plot(alig_v, alig_u,color='g')
...
def plot_distances(signals, **kwargs):
    ...
    fig = plt.figure()
    im = fig.imshow(dist_matrix,cmap=cmap, interpolation='nearest')
    fig.axis('off') if axis else None
...
def plot_cost_matrices(signals, **kwargs):
    VMAX = max( len(s) for s in signals)
